 ...
 * then the original name stored in the "Nick" table should NOT change.
 *
 ...
 *      the `Nick` table)
 *   3. The user's matching `Nick` base_nick is NULL (that is, no gimmick has occurred)
 *
 ...
 *       3.1 If it does not match the `gimmick_nick` entry in the `Nick` table, the
...
test("/set-property", () => {
    ...
    describe("Add user to Nick", () => {
        test("New user to Nick in a new guild", () => {});
    });
});
